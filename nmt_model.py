...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        output, (h_enc, c_enc)  = self.encoder(packed)
        enc_hiddens,lens_unpacked = pad_packed_sequence(output, batch_first = True)
        init_decoder_hidden = self.h_projection(torch.cat((h_enc[0,:],h_enc[1,:]),1))
        init_decoder_cell = self.c_projection(torch.cat((c_enc[0,:],c_enc[1,:]),1))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        # (self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
